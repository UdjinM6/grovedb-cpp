...
int main() {
  ...
  std::string dir = MakeTempDir("rust_batch_parity");
  std::string cmd =
      test_utils::RustToolsCargoRunPrefix() + "rust_storage_batch_writer \"" +
      dir + "\"";
  if (std::system(cmd.c_str()) != 0) {
    Fail("failed to run rust storage batch writer");
  }
  ...
  if (found) {
    Fail("k1 should be deleted in batch");
  }
