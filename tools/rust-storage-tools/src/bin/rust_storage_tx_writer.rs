...
fn main() {
    ...
    let tx = db.transaction_opt(
        &WriteOptions::default(),
        &rocksdb::OptimisticTransactionOptions::default(),
    );
    tx.put_cf(&cf_default, prefixed_key(prefix_root, b"k1"), b"v1")
        .unwrap();
    tx.put_cf(&cf_default, prefixed_key(prefix_root, b"k2"), b"v2")
        .unwrap();
    tx.delete_cf(&cf_default, prefixed_key(prefix_root, b"k1"))
        .unwrap();
    tx.put_cf(&cf_roots, prefixed_key(prefix_root, b"r1"), b"rv1")
        .unwrap();
    if env::args().nth(2).as_deref() == Some("rollback") {
        tx.rollback().unwrap();
        return;
    }
    tx.commit().unwrap();
